...
async function checkScheduledCalls() {
  ...
  const result = await httpsRequest({
    ...
    method: 'GET',
    headers: {
      'apikey': process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_KEY,
      'Authorization': 'Bearer ' + (process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_KEY)
    }
  });
...
// ⬡B:REACH:FIX:dynamic_voicemail:20260305⬡
// Dynamic voicemail messages - generated by AIR with FCW context
...
async function getVoicemailMessage(contactName, context = '') {
  try {
    ...
    const response = await fetch(`${ABACIA_SERVICES_URL}/api/air/process`, {
      ...
      body: JSON.stringify({
        type: 'voicemail_generation',
        message: `Generate a brief, warm voicemail message for ${contactName || 'this person'}. Context: ${context || 'General follow-up call'}. Keep it under 30 words. Friendly butler tone. Start with their name if known. Sign off warmly as ABA.`,
        source: 'REACH_VOICEMAIL',
        channel: 'voice'
      })
    });
    ...
    if (response.ok) {
      ...
      if (data.response && data.response.length > 10) {
        console.log('[VOICEMAIL] AIR generated:', data.response.substring(0, 60));
        return data.response;
      }
    }
  } catch (e) {
    console.error('[VOICEMAIL] AIR generation failed:', e.message);
  }
  
  // Fallback - generic warm message (only if AIR fails)
  const firstName = contactName?.split(' ')[0] || 'there';
  return `Hello ${firstName}, this is ABA reaching out. I was not able to connect with you directly, but wanted to make sure you received this message. When you have a moment, please feel free to return this call. Thank you, and have a wonderful day.`;
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ⬡B:AIR:REACH.VOICE.CONTACTS:CONFIG:contacts.registry:v2.2.0:20260305⬡
// CONTACT REGISTRY - Known callers with trust levels
// L3: Agent-level configuration
// NOTE: Greetings now generated dynamically by AIR (see generateDynamicGreeting)
// ═══════════════════════════════════════════════════════════════════════════════
  ...
  try {
    const response = await fetch(`${ABACIA_SERVICES_URL}/api/air/process`, {
      ...
      body: JSON.stringify({
        ...
        message: `Generate a brief, warm voice greeting for ${name || 'caller'}. Their role: ${role || 'contact'}. Context: ${context || 'phone call'}. Keep under 10 words. Warm butler tone. Natural, not robotic.`,
        source: 'REACH_GREETING',
        channel: 'voice'
      })
    });
    ...
    if (response.ok) {
      const data = await response.json();
      if (data.response && data.response.length > 3 && data.response.length < 100) {
        return data.response;
      }
    }
  } catch (e) {
    console.log('[GREETING] AIR generation failed:', e.message);
  }
  
  // Fallback - simple warm greeting
  const firstName = name?.split(' ')[0] || 'there';
  return `Hello ${firstName}! Good to hear from you.`;
}
...
async function lookupCallerByPhone(phoneNumber) {
  ...
  try {
    ...
    if (brainResult.status === 200) {
      ...
      if (data.length > 0) {
        ...
        // Generate dynamic greeting via AIR (no hardcoded templates)
        const greeting = await generateDynamicGreeting(name, role, 'incoming phone call');
  ...
  for (const [regPhone, contact] of Object.entries(CONTACT_REGISTRY)) {
    ...
    if (regNorm === normalized || regNorm.slice(-10) === last10) {
      ...
      // Generate dynamic greeting instead of using hardcoded one
      const dynamicGreeting = await generateDynamicGreeting(contact.name, contact.role, 'incoming phone call');
      return { ...contact, greeting: dynamicGreeting, phone: phoneNumber, found: true };
    }
  }
  ...
  try {
    ...
    if (result.status === 200) {
      ...
      if (data.length > 0) {
        ...
        const contactName = nameMatch ? nameMatch[1].trim() : 'Contact';
        const contactRole = roleMatch ? roleMatch[1] : 'contact';
        const dynamicGreeting = await generateDynamicGreeting(contactName, contactRole, 'incoming phone call');
  ...
  // Unknown caller - still generate dynamic but guarded greeting
  const unknownGreeting = await generateDynamicGreeting('caller', 'unknown', 'unknown incoming call - need to identify');
  return {
    name: 'Caller',
    role: 'unknown',
    trust: 'T2',
...
async function healthCheck(pulseId) {
  ...
  // ⬡B:HEALTH:ABABASE_KEEPALIVE:20260315⬡ — Ping ABABASE every 5 min to prevent cold starts
  // Without this, ABABASE sleeps on free tier Render. Phone calls hit sleeping server = hang up.
  try {
    await fetch('https://abacia-services.onrender.com/health');
    health.ababase = true;
  } catch (e) {
    health.ababase = false;
    console.log('[PULSE:HEALTH] ABABASE ping failed:', e.message);
  }
...
async function identifyCaller(phoneNumber) {
  if (CONTACT_REGISTRY[phoneNumber]) {
    ...
    // Generate dynamic greeting instead of using hardcoded
    const greeting = await generateDynamicGreeting(c.name, c.role, 'incoming phone call');
    return { ...c, greeting };
  }
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      const greeting = await generateDynamicGreeting('Known Contact', 'contact', 'caller found in brain');
  ...
  const unknownGreeting = await generateDynamicGreeting('Unknown', 'unknown', 'unknown caller - need to identify');
...
async function getGreetingForCaller(callerIdentity, callHistory, touchpoints) {
  // ALL greetings now generated dynamically by AIR
  // No more hardcoded greetings for anyone
  
  let context = '';
  let callerName = 'there';
  
  if (touchpoints.type === 'owner') {
    context = 'This is the owner/creator calling. Be warm, direct, efficient.';
    callerName = 'Brandon';
  } else if (touchpoints.type === 'returning') {
    const nameMatch = callHistory?.match(/CALL SUMMARY \| (\w+)/);
    callerName = nameMatch ? nameMatch[1] : 'there';
    touchpoints.callerName = callerName;
    context = 'This is a returning caller we have spoken with before. Welcome them back warmly.';
  } else if (touchpoints.type === 'known') {
    context = 'This is a known contact. Warm and professional.';
  } else {
    context = 'This is a first-time caller. Give a warm welcome and introduce yourself as ABA. Ask for their name.';
  }
  ...
  try {
    const response = await fetch(`${ABACIA_SERVICES_URL}/api/air/process`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        type: 'voice_greeting_generation',
        message: `Generate a warm phone greeting for ${callerName}. Caller type: ${touchpoints.type}. Context: ${context}. Keep conversational. If first-time caller, introduce yourself as ABA and ask their name. Butler tone, warm not robotic.`,
        source: 'REACH_VOICE_GREETING',
        channel: 'voice'
      })
    });
    ...
    if (response.ok) {
      const data = await response.json();
      if (data.response && data.response.length > 10 && data.response.length < 500) {
        return data.response;
      }
    }
  } catch (e) {
    console.log('[GREETING] AIR generation failed, using fallback:', e.message);
  }
  
  // Fallback only if AIR fails completely
  if (touchpoints.type === 'owner') {
    return `Hello ${callerName}! What can I help you with?`;
  } else if (touchpoints.type === 'returning') {
    return `Hello ${callerName}! Good to hear from you again.`;
  } else if (touchpoints.type === 'known') {
    return `Hello! This is ABA. How can I help you today?`;
  } else {
    return `Hello! Welcome. My name is ABA. Who do I have the pleasure of speaking with?`;
  }
}
...
function connectDeepgram(session) {
  ...
  ws.on('open', () => {
    ...
    fetch(`${SUPABASE_URL}/rest/v1/aba_memory`, {
      method: 'POST',
      headers: { 'apikey': process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_KEY, 'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
...
const httpServer = http.createServer(async (req, res) => {
  ...
  // wake detection, TASTE classification, and brain writes. No processing in REACH.
  // ═══════════════════════════════════════════════════════════════════════
      ...
      setImmediate(async () => {
        try {
          const fwd = await fetch(targetUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-omi-forwarded-by': 'reach-proxy' },
            body: JSON.stringify(body)
          });
          console.log('[OMI-PROXY] forwarded status=' + fwd.status + ' url=' + targetUrl);
          broadcastToCommandCenter({ type: 'omi_forwarded', target: 'ababase', url: targetUrl, status: fwd.status });
        } catch (fwdErr) {
          console.error('[OMI-PROXY] forward failed:', fwdErr.message);
        }
      });
      return;
    } catch (e) {
...
httpServer.listen(PORT, '0.0.0.0', () => {
  ...
  // ⬡B:heartbeat:REENABLED:gemini:20260309⬡
  // RE-ENABLED WITH GEMINI FLASH (FREE)
  startHeartbeat();
  console.log('[HEARTBEAT] DISABLED - Heartbeats burn Claude credits');
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // ⬡B:pulse:REENABLED:gemini:20260309⬡
  // RE-ENABLED WITH GEMINI FLASH (FREE)
  console.log('[PULSE] DISABLED - Pulse heartbeat burns Claude credits');
  startPulseHeartbeat();
